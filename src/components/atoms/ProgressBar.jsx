...
const ProgressBar = ({ current, total, className = '' }) => {
  ...
  return (
    <div className={`w-full bg-white rounded-2xl p-6 shadow-xl border-2 border-sky-100 ${className}`}>
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          <div className="bg-sky-300 rounded-lg p-2 shadow-md">
            <ChartLineUpIcon size={24} weight="fill" className="text-slate-800" />
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Progress</p>
            <p className="text-sm font-bold text-gray-700">Question {current} of {total}</p>
          </div>
        </div>
        ...
        >
          ...
          {percentage > 15 && (
            <div className="absolute right-2 top-1/2 -translate-y-1/2">
              <CheckIcon size={16} weight="fill" className="text-white drop-shadow" />
            </div>
          )}
        </motion.div>
      </div>
